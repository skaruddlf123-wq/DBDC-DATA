...
# -----------------------------------------------------------------------------
# 3. 단일 기업 데이터 로딩 함수 (속도 최적화: 선택 기업 1개만 즉시 로드)
# -----------------------------------------------------------------------------
...
# 전체 50개사 요약 함수 (페이지 2 전용)
@st.cache_data(ttl=3600)
def get_all_companies_summary():
    summary_list = []
    progress_bar = st.progress(0, text="50개 기업 데이터를 수집하는 중입니다...")
    ...
    for idx, comp in enumerate(COMPANY_LIST):
        ...
        progress_bar.progress((idx + 1) / len(COMPANY_LIST), text=f"50개 기업 수집 중... ({idx+1}/{len(COMPANY_LIST)})")
    ...
    if selected_company == "-- 기업을 선택하세요 --":
        ...
        <div class="welcome-card">
            ...
            <p style="font-size:1.05rem; color:#444444; line-height:1.6;">
                ...
                해당 기업의 DB 가입자 명부 데이터 및 <b>임금피크 도래 대상자 영업 명단</b>이 즉시 집계되어 표출됩니다.
            </p>
        ...
        with st.spinner(f"{selected_company} 데이터를 빠르게 로딩 중입니다..."):
        ...
        if df is not None:
            ...
            # 단일 기업 집계
            tot_cnt = len(active_df)
            tot_ob = active_df['퇴직금추계액'].sum()
            avg_ob = active_df['퇴직금추계액'].mean() if tot_cnt > 0 else 0
            
            target_df = active_df[(active_df['만나이'] >= target_min_age) & (active_df['만나이'] <= target_max_age)].copy()
            target_cnt = len(target_df)
            comp_target_ratio = (target_cnt / tot_cnt * 100) if tot_cnt > 0 else 0
            target_avg_ob = target_df['퇴직금추계액'].mean() if target_cnt > 0 else 0
            
            # 전체 50개사 평균 비중 (고정 벤치마크값: 16.8%)
            avg_target_ratio_50 = 16.8
...
# =============================================================================
    ...
    with st.spinner("50개 기업 전체 데이터를 동기화 중입니다..."):
